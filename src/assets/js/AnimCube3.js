...
function AnimCube3(HTML_element, params) {
  ...
  function init() {
    ...
    if (param != null) {
      var rm = 0;
      for (var i = 0; i < param.length; i++)
        if (param.charAt(i) >= '0' && param.charAt(i) <= '9') rm = rm * 10 + parseInt(param[i]);
    ...
    // CUSTOM
    // function setEyeFromInitialPosition(param, eye, eyeX) {
    //   var pi12 = Math.PI / 12;
    //   for (var i = 0; i < param.length; i++) {
    //     var angle = pi12;
    ...
    setEyeFromInitialPosition(param, eye, eyeX);
  ...
  function getMovePart(sequence, info, num) {
    ...
    if (sequence.trim() == '#') if (typeof move[num] != 'undefined') return move[num];
    var length = 0;
    var mv = [];
  ...
  function moveLength(move, end) {
    var length = 0;
    for (var i = 0; i < move.length && (i < end || end < 0); i++) length += turnLength(move[i]);
    return length;
  }
  ...
  function twistLayer2(cube, layer, num, middle) {
    ...
    for (var i = 0; i < 4; i++) {
      ...
      for (var j = 0; j < 3; j++, k++) twistBuffer[k % 12] = cube[n][j * factor + offset];
    }
    for (var i = 0, k = 0; i < 4; i++) {
      ...
      var factor = cycleFactors[c];
      var offset = cycleOffsets[c];
      for (var j = 0; j < 3; j++, k++) cube[n][j * factor + offset] = twistBuffer[k];
    }
  }
  ...
  */
  ...
  function twistSuperLayer(layer, num, mode) {
    if (mode == 0) {
      // outer
      for (var i = 0; i < 9; i++) scube[layer][i] = (scube[layer][i] + 4 - num) % 4;
  ...
  function superTwist1(v) {
    for (var i = v[0], n = 0; n < 3; i += v[1], n++) scube[v[2]][i] = (scube[v[2]][i] + 2) % 4;
  }

  function superTwist2(ix, tw) {
    for (var i = 0; i < 4; i++) {
      var v = superTwistArr[ix][i];
      for (var j = v[0], n = 0; n < 3; j += v[1], n++) scube[v[2]][j] = (scube[v[2]][j] + tw) % 4;
    }
  }
  ...
  function paint() {
    ...
    var h =
      buttonBar == 1 && (progressHeight == 0 || demo || move[curMove].length == 0)
        ? height - dpr
        : height;
    ...
    if (natural) {
      ...
      for (var i = 0; i < 3; i++) {
        ...
        for (var j = 0; j < 3; j++) {
          ...
          tempEye[i] +=
            eye[j] * (rotVec[axis][i][j] + rotCos[axis][i][j] * cosA + rotSin[axis][i][j] * sinA);
          tempEyeX[i] +=
            eyeX[j] * (rotVec[axis][i][j] + rotCos[axis][i][j] * cosA + rotSin[axis][i][j] * sinA);
        }
      }
      ...
      for (var i = 0; i < 3; i++) {
        ...
        for (var j = 0; j < 3; j++) {
          var axis = Math.floor(twistedLayer / 2);
          tempEye2[i] +=
            eye[j] * (rotVec[axis][i][j] + rotCos[axis][i][j] * cosB + rotSin[axis][i][j] * sinB);
          tempEyeX2[i] +=
            eyeX[j] * (rotVec[axis][i][j] + rotCos[axis][i][j] * cosB + rotSin[axis][i][j] * sinB);
        }
      }
      ...
      if (topProd < 0 && botProd > 0)
        ...
        orderMode = 1; // both top and bottom layer facing away: draw them first
      else orderMode = 2;
      for (var i = 0; i < 3; i++) {
        var j = drawOrder[orderMode][i];
        var k = eyeOrder[twistedMode][j];
        fixBlock(eyeArray[k], eyeArrayX[k], eyeArrayY[k], blockArray[j], blockMode[twistedMode][j]);
      }
    }
    if (!pushed && !animating)
      ...
      if (!demo && move.length > 0) {
        if (move[curMove].length > 0) {
          ...
          if (progressHeight > 0) {
            ...
            var progress =
              ((width - 2) * realMovePos(move[curMove], movePos)) / realMoveLength(move[curMove]);
          ...
          if (moveText > 0) {
            if (moveCounter) drawString(graphics, s, outlined ? x - dpr : x, y - textHeight);
            drawMoveTextFunc(graphics, y);
          } else if (moveCounter) drawString(graphics, s, outlined ? x - dpr : x, y);
        }
        if (move.length > 1) {
          // more sequences
          graphics.font = 'bold ' + textHeight + 'px helvetica';
  ...
  function fixBlock(eye, eyeX, eyeY, blocks, mode) {
    ...
    for (var i = 0; i < 8; i++) {
      ...
      if (align == 0) coordsY[i] = ((height - progressHeight) / 2.0) * scale - y;
            ...
            for (var n = 0, p = blocks[i][1][0]; n < sideH; n++, p++) {
              for (var o = 0, q = blocks[i][0][0]; o < sideW; o++, q++) {
                ...
                if (superCube == true) {
                  ...
                  if (hintBorder) drawPolygon(graphics, fillX, fillY, darker('#fdfdfd'));
                  else drawPolygon(graphics, fillX, fillY, '#fdfdfd');
                  drawSuperArrow(
                    graphics,
                    fillX,
                    fillY,
  ...
  function rgbToHex(r, g, b) {
    return '#' + lpad(r.toString(16)) + lpad(g.toString(16)) + lpad(b.toString(16));
  }
  ...
  function drawButtonsFunc(g) {
    ...
    if (buttonBar == 1) {
      ...
      for (var i = 0; i < 7; i++) {
        ...
        g.strokeStyle = buttonBorderColor;
        g.beginPath();
        if (i == 0) g.rect(buttonX + dph, height - dph, buttonWidth - dpr, buttonHeight);
    ...
    function callback() {
      if (outerLoopTop) {
        ...
        if (repeatable) {
          ...
          if ((moveDir > 0 && movePos >= mv.length) || (moveDir < 0 && movePos == 0)) {
            restarted = false;
            animating = false;
            nowServing++;
            drawButtons = true;
      ...
      if (innerLoopTop) {
        ...
        if (!cont) {
          ...
          } else if (mv[movePos] >= 1000) curInfoText = moveDir > 0 ? mv[movePos] - 1000 : -1;
          else spin = true;
          if (spin) {
            num = (mv[movePos] % 4) + 1;
  ...
  function mousedown(e) {
    ...
    if (buttonPressed >= 0) button();
      ...
      if (typeof e.touches == 'undefined') {
        if (editable && !animating && e.button == 0 && !e.shiftKey) toTwist = true;
      } else {
  ...
  function button() {
    ...
    if (buttonPressed == 3) {
      ...
      if (!animating) {
        ...
        setEyeFromInitialPosition(paramNew, eye, eyeX);
      ...
      stopAnimation();
      setTimeout(clear, 0);
      if (buttonPressed == 7) curMove = curMove > 0 ? curMove - 1 : move.length - 1;
  ...
  function scaleCanvas() {
    ...
    for (var i = 0; i < textOffsetInit.length; i++) textOffset[i] = textOffsetInit[i] * dpr;
  ...
  function parseSearchParams() {
    ...
    for (var i = 0; i < lines.length; i++) {
      var line = lines[i].split('=');
      if (typeof line[1] != 'undefined') searchParams[line[0]] = decodeURIComponent(line[1].trim());
    }
  }
  ...
  function init0() {
    ...
    //   parNode.innerHTML = '';
    // } else {
    //   if (document.currentScript != null) parNode = document.currentScript.parentNode;
    //   else {
  ...
  document.addEventListener('keydown', (event) => {
    ...
    if (arrows.includes(event.key)) {
      ...
      lastX = o;
      lastY = a;
      currentAngle = (0.01 * (dragX * i + dragY * n)) / Math.sqrt(dragX * dragX + dragY * dragY);
      paint();
    }
  });
